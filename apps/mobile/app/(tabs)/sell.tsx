...
import {
  ...
  CATEGORY_EMOJI, LISTING_CONDITIONS, CLOTHING_SIZES, GRADES, SA_PROVINCES,
} from '@nextkid/shared';
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.inner} keyboardShouldPersistTaps="handled">
        ...
        {step === 2 && isSchoolSpecific && <>
          ...
          <Text style={styles.stepSub}>Link this listing to a school so the right buyers find it.</Text>
          ...
          {province && <>
            ...
            <TextInput style={styles.input} value={schoolSearch} onChangeText={setSchoolSearch} placeholder="Type school name..." placeholderTextColor="#555" />
                ...
                : filteredSchools.length === 0
                  ...
                  : filteredSchools.map(school => (
                    <TouchableOpacity key={school.id} onPress={() => setSelectedSchool(school)}
                      ...
                      {selectedSchool?.id === school.id && <Text style={{ color: '#a855f7', fontSize: 16 }}>✓</Text>}
                    </TouchableOpacity>
                  ))
              }
            </View>
        ...
        {step === 3 && <>
          ...
          <TextInput style={styles.input} value={title} onChangeText={setTitle} placeholder="e.g. Grey flannel trousers size 32" placeholderTextColor="#555" />
          ...
          <TextInput style={styles.input} value={price} onChangeText={setPrice} placeholder="250" placeholderTextColor="#555" keyboardType="numeric" />

          <Text style={styles.label}>Description</Text>
          <TextInput style={[styles.input, { height: 90, textAlignVertical: 'top' }]} value={description} onChangeText={setDescription}
            placeholder="Condition, why you're selling, any defects..." placeholderTextColor="#555" multiline />
...
function SummaryRow({ label, value }: { label: string; value: string }) {
  return (
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: '#222' }}>
      <Text style={{ color: '#888', fontSize: 13 }}>{label}</Text>
      <Text style={{ color: '#fff', fontSize: 13, fontWeight: '600', flex: 1, textAlign: 'right' }}>{value}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  stepTitle: { color: '#fff', fontSize: 18, fontWeight: '700', marginBottom: 4 },
  stepSub: { color: '#888', fontSize: 13, marginBottom: 20 },
  categoryGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 20 },
  categoryCard: {
    width: '47%', padding: 14, borderRadius: 12,
    backgroundColor: '#111', borderWidth: 1.5, borderColor: '#222',
  },
  categoryCardActive: { borderColor: '#7c3aed', backgroundColor: '#2e1f5e40' },
  categoryEmoji: { fontSize: 28, marginBottom: 6 },
  categoryName: { color: '#aaa', fontSize: 13, fontWeight: '600' },
  categoryNameActive: { color: '#fff' },
  schoolTag: { color: '#7c3aed', fontSize: 10, marginTop: 2 },
  label: { color: '#aaa', fontSize: 12, marginBottom: 6, marginTop: 8 },
  input: {
    backgroundColor: '#1a1a1a', borderWidth: 1, borderColor: '#2a2a2a',
    borderRadius: 10, padding: 12, color: '#fff', fontSize: 14, marginBottom: 4,
  },
  ...
  selectedBanner: {
    ...
    backgroundColor: '#2e1f5e40', borderWidth: 1, borderColor: '#7c3aed',
    borderRadius: 10, padding: 12, marginBottom: 16,
  },
  selectedBannerText: { color: '#a78bfa', fontSize: 13, fontWeight: '600' },
  summaryBox: { backgroundColor: '#111', borderRadius: 12, padding: 16, borderWidth: 1, borderColor: '#222', marginBottom: 20 },
  ...
  successSub: { color: '#888', fontSize: 14, marginBottom: 32, textAlign: 'center' },
});
