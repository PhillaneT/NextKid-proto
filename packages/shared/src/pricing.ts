 ...
 *   Step 2: + delivery_fee                      (fixed school delivery fee)
 *   Step 3: ÷ (1 - platform_rate)               (gross up for NextKid markup)
 *   Step 4: ÷ (1 - gateway_rate)                (gross up for Stitch fee)
 *   Step 5: CEILING(raw ÷ roundTo) × roundTo    (round UP to nearest R25)
...
export interface PriceBreakdown {
  ...
  deliveryFeeCents:     number   // Step 2 — school delivery fee
  subtotalCents:        number   // Step 2 result
  platformFeeCents:     number   // Step 3 — NextKid markup (grossed up)
  afterMarkupCents:     number   // Step 3 result
  gatewayFeeCents:      number   // Step 4 — Stitch fee (grossed up)
  buyerRawCents:        number   // Step 4 result — before rounding
  buyerPriceCents:      number   // Step 5 — final rounded buyer price
  adminFeeCents:        number   // Step 6 — rounding surplus (Praesignis Finance)
...
export function calculateBuyerPrice(
  ...
  // Step 2 — add fixed delivery fee
  const deliveryFeeCents  = Math.round(config.deliveryFeeRands * 100)
  const subtotalCents     = sellerPayoutCents + deliveryFeeCents

  // Step 3 — gross up for NextKid platform markup
  // subtotal ÷ (1 - platformRate) → buyer funds the markup
  const afterMarkupCents  = Math.round(subtotalCents / (1 - config.platformRate))
  const platformFeeCents  = afterMarkupCents - subtotalCents

  // Step 4 — gross up for Stitch gateway fee
  ...
  // Step 5 — round UP to nearest R25
  const roundCents        = config.roundTo * 100
  const buyerPriceCents   = roundUpTo(buyerRawCents, roundCents)

  // Step 6 — admin fee is the rounding surplus (goes to Praesignis Finance)
